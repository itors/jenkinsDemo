...
<title>用户信息 - 用户管理- 成绩信息管理系统 v1.0</title>
	...
	<div class="Hui-article">
		...
		<div class="text-c">
			<form action="" id="search1">
				用户：<input type="text" name="userName" id="userName" placeholder=" 用户名" style="width:250px" class="input-text">
					 角色： <select class="select" size="1" id="roleId" name="roleId" style="width:200px;height:30px;">
					    <option value="" selected>请选择角色</option>
					    <option value="1">管理员</option>
					    <option value="2">教师</option>
					    <option value="3">学生</option>
					  </select>
				<button name="search" id="search" class="btn btn-success" type="button" onclick="searchUser()"><i class="Hui-iconfont">&#xe665;</i> 搜用户</button>
			</form>
			</div>
			<div class="cl pd-5 bg-1 bk-gray mt-20">
				<span class="l">
				<a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
				<a class="btn btn-primary radius" data-title="添加用户" onclick="article_add('添加用户','<%=basePath%>/user/userAddPage.action','500','350')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加用户</a>
				</span>
			</div>
			<div class="mt-20">
				<table class="table table-border table-bordered table-hover table-bg table-sort">
					...
					<tbody>
						...
						<tr class="text-c">
							<td><input type="checkbox" value="${v.userId}" name="box"></td>
							<td>${v.userId}</td>
							<td>${v.userName}</td>
							<td>${v.roleId}</td>
							<td>${v.userCode}</td>
							...
							<td class="f-14 td-manage">
								<a style="text-decoration:none" class="ml-5" onClick="article_edit('用户编辑','<%=basePath%>/user/userEditPage.action?userId=${v.userId}','10001','500','350')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
								<a style="text-decoration:none" class="ml-5" onClick="article_del(this,'${v.userId}')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
						</tr>
						</c:forEach>
					</tbody>
				</table>
...
function article_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			data: {userId:id},
			url: '${basePath}/user/delUser.action',
			dataType: 'json',
...
function datadel(){
	...
	else
	{
		layer.confirm('确认要删除吗？',function(index){
			$.ajax({
				type: 'POST',
				data: {ids:chestr,tb:'t_user',id:'user_id'},
...
//     });
	...
	window.location.href = "${basePath}/user/allUser.action?userName="+userName+"&roleId="+roleId;
