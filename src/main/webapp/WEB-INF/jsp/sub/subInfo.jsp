...
<aside class="Hui-aside">
	...
	<c:if  test  = "${sessionScope.roleId eq '1'||sessionScope.roleId eq '2'}"> 
		...
		<c:if  test  = "${sessionScope.roleId eq '1'}"> 
		<dl id="menu-product">
			<dt class="selected"><i class="Hui-iconfont">&#xe601;</i> 科目管理<i class="Hui-iconfont menu_dropdown-arrow">&#xe6d5;</i></dt>
	...
	<div class="Hui-article">
		<article class="cl pd-20">
			<div class="cl pd-5 bg-1 bk-gray mt-20">
				...
				<a class="btn btn-primary radius" data-title="添加科目" onclick="article_add('添加科目','<%=basePath%>/sub/subAddPage.action','500','350')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加科目</a>
				</span>
			</div>
			<div class="mt-20">
				<table class="table table-border table-bordered table-hover table-bg table-sort">
					...
					<tbody>
						...
						<tr class="text-c">
							<td><input type="checkbox" value="${v.sId }" name="box"></td>
							<td>${v.sId}</td>
							<td>${v.sName}</td>
							<td class="f-14 td-manage">
								<a style="text-decoration:none" class="ml-5" onClick="article_edit('科目编辑','<%=basePath%>/sub/subEditPage.action?sId=${v.sId}','10001','500','350')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
								<a style="text-decoration:none" class="ml-5" onClick="article_del(this,'${v.sId}')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
						</tr>
						</c:forEach>
					</tbody>
				</table>
...
function article_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			data: {sId:id},
			url: '${basePath}/sub/delSub.action',
			dataType: 'json',
...
function datadel(){
	...
	else
	{
		layer.confirm('确认要删除吗？',function(index){
			$.ajax({
				type: 'POST',
				data: {ids:chestr,tb:'t_subject',id:'s_id'},
