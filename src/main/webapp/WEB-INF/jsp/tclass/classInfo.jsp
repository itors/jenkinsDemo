...
<title>角色信息 - 角色管理- 成绩信息管理系统 v1.0</title>
...
<aside class="Hui-aside">
	...
	<c:if  test  = "${sessionScope.roleId eq '1'||sessionScope.roleId eq '2'}"> 
		...
		<c:if  test  = "${sessionScope.roleId eq '1'}"> 
		<dl id="menu-product">
			<dt class="selected"><i class="Hui-iconfont">&#xe637;</i> 班级管理<i class="Hui-iconfont menu_dropdown-arrow">&#xe6d5;</i></dt>
	...
	<div class="Hui-article">
		<article class="cl pd-20">
			<div class="cl pd-5 bg-1 bk-gray mt-20">
				...
				<a class="btn btn-primary radius" data-title="添加班级" onclick="article_add('添加班级','<%=basePath%>/tclass/tclassAddPage.action','500','350')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加班级</a>
				</span>
			</div>
			<div class="mt-20">
				<table class="table table-border table-bordered table-hover table-bg table-sort">
					...
					<tbody>
						...
						<tr class="text-c">
							<td><input type="checkbox" value="${v.cId }" name="box"></td>
							<td>${v.cId}</td>
							<td>${v.major}</td>
							<td>${v.className}</td>
							<td class="f-14 td-manage">
								<a style="text-decoration:none" class="ml-5" onClick="article_edit('角色编辑','<%=basePath%>/tclass/tclassEditPage.action?cId=${v.cId}','10001','500','350')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
								<a style="text-decoration:none" class="ml-5" onClick="article_del(this,'${v.cId}')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
						</tr>
						</c:forEach>
					</tbody>
				</table>
...
function article_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			data: {cId:id},
			url: '${basePath}/tclass/delTClass.action',
			dataType: 'json',
...
function datadel(){
	...
	else
	{
		layer.confirm('确认要删除吗？',function(index){
			$.ajax({
				type: 'POST',
				data: {ids:chestr,tb:'t_class',id:'c_id'},
