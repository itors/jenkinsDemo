...
<title>成绩信息 - 成绩管理- 成绩信息管理系统 v1.0</title>
	...
	<div class="Hui-article">
		<article class="cl pd-20">
			...
			<form action="" id="search1">
				学号：<input type="text" name="stuId" id="stuId" placeholder=" 学号" style="width:250px" class="input-text">
				姓名：<input type="text" name="stuName" id="stuName" placeholder=" 学生姓名" style="width:250px" class="input-text">
				学期：<input type="text" name="time" id="time" placeholder=" 学期" style="width:250px" class="input-text">
				<button name="search" id="search" class="btn btn-success" type="button" onclick="searchScore()"><i class="Hui-iconfont">&#xe665;</i> 搜成绩</button>
			</form>
			</div>
			<div class="cl pd-5 bg-1 bk-gray mt-20">
				<span class="l">
				<a href="javascript:;" onclick="datadel()"  class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
				<a class="btn btn-primary radius" data-title="添加成绩" 
				onclick="article_add('添加成绩','<%=basePath%>/score/scoreAddPage.action','800','600')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加成绩</a>
				<button class="btn btn-success radius" data-title="上传成绩" type="button" id="upButton" 
				onclick="uplodeScore('上传成绩','<%=basePath%>/score/scoreUploadPage.action','800','400')" >
				<i class="Hui-iconfont">&#xe642;</i>&nbsp;上传成绩&nbsp;&nbsp;</button>
				<c:if  test  = "${sessionScope.roleId eq '1'||sessionScope.roleId eq '2'}"> 
				<a class="btn btn-primary radius" data-title="导出成绩" href="<%=basePath %>/score/exportExcel.action">
				<i class="Hui-iconfont">&#xe641;</i> 导出成绩</a>
				</c:if>
				</span>
			</div>
			<div class="mt-20">
				<table class="table table-border table-bordered table-bg table-hover table-sort">
					...
					<c:forEach items="${list }" var="v" varStatus="status">
						<tr class="text-c">
							<td><input type="checkbox" value="${v.scoreId}" name="box"></td>
							<td>${v.stuId}</td>
							...
							<td>${v.modifyTime}</td>
							<td>
<!-- 							<c:if  test  = "${v.review eq '1'}">  -->
<!-- 							<span class="label label-success radius">已审核</span> -->
							
<!-- 							</c:if> -->
							<c:choose>
							   <c:when test="${v.review eq '1'}">  
							        <span class="label label-success radius">已审核</span>    
							   </c:when>
							   <c:when test="${v.review eq '2'}">  
							        <span class="label label-danger radius">未通过</span>    
							   </c:when>
							   <c:otherwise> 
							     <span class="label label-defualt radius">未审核</span>
							   </c:otherwise>
							</c:choose>
							...
							<td class="f-14 td-manage">
								<c:if  test  = "${sessionScope.roleId eq '1'}"> 
									<c:choose>
									   ...
									   <c:otherwise> 
									    <a style="text-decoration:none" class="ml-5" onClick="article_shenhe('成绩审核','${v.scoreId}')" href="javascript:;" title="审核">审核</a>
									   </c:otherwise>
									</c:choose>
								</c:if>
								<a style="text-decoration:none" class="ml-5" onClick="article_edit('成绩编辑','<%=basePath%>/score/scoreEditPage.action?scoreId=${v.scoreId}','10001','800','500')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
								<a style="text-decoration:none" class="ml-5" onClick="article_del(this,'${v.scoreId}')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
						</tr>
						</c:forEach>
					</tbody>
				</table>
...
function article_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			data: {scoreId:id},
			url: '${basePath}/score/delScore.action',
			dataType: 'json',
...
function datadel(){
	...
	else
	{
		layer.confirm('确认要删除吗？',function(index){
			$.ajax({
				type: 'POST',
				data: {ids:chestr,tb:'t_score',id:'score_id'},
...
function searchScore(){
	...
	window.location.href = "${basePath}/score/allScore.action?stuId="+stuId+"&stuName="+stuName+"&time="+time;
...
// 		$.ajax({
// 			type: 'POST',
// 			data: {scoreId:id,review:'1'},
// 			url: '${basePath}/score/shenheScore.action',
// 			dataType: 'json',
// 			success: function(data){
// 				parent.location.reload();
// 				$(obj).parents("tr").remove();
